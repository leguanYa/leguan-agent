...
//                        new ReReadingAdvisor()
    ...
    @jakarta.annotation.Resource
    private VectorStore redisVectorVectorStore;


    public String doChatWithRedisVectorRag(String message, String chatId) {
        ...
        ChatResponse response = chatClient
                ...
                .advisors(new MyLoggerAdvisor())
                // 应用增加检索服务（基于PgVector向量存储）
                .advisors(QuestionAnswerAdvisor.builder(redisVectorVectorStore).build(), QuestionAnswerAdvisor.builder(pgVectorVectorStore).build())
                .call()
                .chatResponse();
        String content = response.getResult().getOutput().getText();
        Integer totalTokens = response.getMetadata().getUsage().getTotalTokens();
